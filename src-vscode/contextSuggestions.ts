...
export interface SuggestedContextFile {
  readonly path: string;
  readonly relativePath: string;
  readonly reason: string;
  readonly source: "heuristic" | "neighborhood";
...
export async function suggestContextFiles(
  ...
  sourceText: string
): Promise<readonly SuggestedContextFile[]> {
...
function buildQueryTokens(workflow: UserStoryWorkflowDetails, sourceText: string): readonly string[] {
  const text = [
    workflow.title,
    workflow.category,
...
async function collectWorkspaceCandidatesAsync(workspaceRoot: string): Promise<readonly FileCandidate[]> {
...
function scoreHeuristicSuggestions(
  candidates: readonly FileCandidate[],
  tokens: readonly string[],
  alreadyAttached: ReadonlySet<string>
): SuggestedContextFile[] {
  const matches: SuggestedContextFile[] = [];
...
function scoreCandidate(candidate: FileCandidate, tokens: readonly string[]): number {
  let score = 0;
...
function buildHeuristicReason(candidate: FileCandidate, tokens: readonly string[]): string {
...
function buildNeighborhoodSuggestions(
  candidates: readonly FileCandidate[],
  heuristicSuggestions: readonly SuggestedContextFile[],
  alreadyAttached: ReadonlySet<string>
): SuggestedContextFile[] {
  ...
  const suggestions: SuggestedContextFile[] = [];
...
function collectSameDirectoryNeighbors(
  candidates: readonly FileCandidate[],
  anchor: FileCandidate
...
function collectCounterparts(
  ...
  return candidates.filter((candidate) => {
    ...
    const directoryPair =
      (directory.includes("/tests") && candidateDirectory.includes("/src"))
