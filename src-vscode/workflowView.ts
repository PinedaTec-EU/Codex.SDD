...
export function buildWorkflowHtml(
  ...
  const artifactSection = selectedPhase.artifactPath
    ? isClarificationDetail
      ? buildArtifactPreviewSection(
        selectedIteration?.artifactPath ?? selectedPhase.artifactPath,
        artifactPreviewHtml,
...
function buildRoundedOrthogonalPath(
  ...
  const cleanPoints = [start, ...points].filter((point, index, list) =>
  ...
  for (let index = 1; index < cleanPoints.length; index++) {
    ...
    const radius = Math.min(
      24,
      Math.abs(current.x - previous.x) / 2 || 24,
      Math.abs(current.y - previous.y) / 2 || 24,
      Math.abs(next.x - current.x) / 2 || 24,
      Math.abs(next.y - current.y) / 2 || 24
    );
