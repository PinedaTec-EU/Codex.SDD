...
public sealed class WorkflowRunner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            EnsureReviewCanConsumeImplementationEvidence(workspaceRoot, paths);
        }
        ...
        var implementationEvidenceBaseline = workflowRun.CurrentPhase == PhaseId.Implementation
            ? await TryCaptureWorkspaceSnapshotAsync(workspaceRoot, paths.RootDirectory, cancellationToken)
            : null;
        ...
        {
            var implementationEvidence = await CaptureImplementationEvidenceAsync(
                workspaceRoot,
                paths,
                implementationEvidenceBaseline,
                cancellationToken);
            await PersistImplementationEvidenceAsync(paths, implementationEvidence, cancellationToken);
            await File.WriteAllTextAsync(
                artifactPath,
                AppendImplementationEvidenceSection(
                    result.Content,
    ...
    private static void EnsureReviewCanConsumeImplementationEvidence(string workspaceRoot, UserStoryFilePaths paths)
    {
        ...
        var evidence = JsonSerializer.Deserialize<ImplementationPhaseEvidenceDocument>(payload, WorkflowJsonSerializerOptions)
    ...
    private static async Task<ImplementationPhaseEvidenceDocument> CaptureImplementationEvidenceAsync(
        string workspaceRoot,
        UserStoryFilePaths paths,
        IReadOnlyCollection<WorkspaceSnapshotEntry>? baselineSnapshot,
        CancellationToken cancellationToken)
    {
        var currentSnapshot = await TryCaptureWorkspaceSnapshotAsync(workspaceRoot, paths.RootDirectory, cancellationToken);
    ...
    private static async Task PersistImplementationEvidenceAsync(
        UserStoryFilePaths paths,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var payload = JsonSerializer.Serialize(evidence, WorkflowJsonSerializerOptions);
        await File.WriteAllTextAsync(jsonPath, payload, cancellationToken);
        await File.WriteAllTextAsync(markdownPath, RenderImplementationEvidenceMarkdown(evidence), cancellationToken);
    }

    private static string AppendImplementationEvidenceSection(
    ...
    private static string RenderImplementationEvidenceMarkdown(ImplementationPhaseEvidenceDocument evidence)
    {
    ...
    private static async Task<IReadOnlyCollection<WorkspaceSnapshotEntry>?> TryCaptureWorkspaceSnapshotAsync(
        string workspaceRoot,
    ...
    private static readonly JsonSerializerOptions WorkflowJsonSerializerOptions = new()
    {
        WriteIndented = true
    };

    private sealed record WorkspaceSnapshotEntry(
        string StatusCode,
        string PathDisplay,
        IReadOnlyCollection<string> CandidatePaths,
        string Fingerprint);

    private sealed record ImplementationPhaseEvidenceDocument(
        string GeneratedAtUtc,
        IReadOnlyCollection<string> Summary,
        IReadOnlyCollection<ImplementationPhaseTouchedFile> TouchedFiles);

    private sealed record ImplementationPhaseTouchedFile(
        string Path,
        string ChangeKind,
